...
public:
    ...
    Relation join(const Relation& right) {
        ...
        // for(int i = 0; i < left.scheme.size(); i++) {
        //     cout << "Scheme: " << left.scheme.at(i) << endl;
        //     for(int j = 0; j < right.scheme.size(); j++) {
        //         cout << "Right scheme: " << right.scheme.at(j) << endl;
        //     }
        // }
        return result;
    }
    ...
    static bool joinable(const Scheme& leftScheme, const Scheme& rightScheme, const Tuple& leftTuple, const Tuple& rightTuple) {
        ...
        // Test to see if the schemes actually do have a similar attribute
        for (size_t i = 0; i < leftScheme.size(); i++) {
            for (size_t j = 0; j < rightScheme.size(); j++) {
                if(leftScheme.at(i) == rightScheme.at(j)) {
                    return true;
